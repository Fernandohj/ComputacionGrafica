//Práctica 12                        Hernández Juárez Fernando 	
//Fecha de entrega: 3 de mayo de 2026    320115448
#include <iostream>
#include <cmath>
#include <fstream> // Libreria para leer y escribir archivos de texto
...
void interpolation(void)
{
	...
	KeyFrame[playIndex].rollDogInc = (KeyFrame[playIndex + 1].rollDog - KeyFrame[playIndex].rollDog) / i_max_steps; // NUEVO: Interpolacion lateral
}

// Funciones para guardar y cargar en archivo físico
void saveToFile() {
	...
	if (file.is_open()) {
		...
		file.close();
		printf("¡Animación guardada exitosamente en 'animacion.txt'!\n");
	}
	else {
...
void loadFromFile() {
	...
	if (file.is_open()) {
		...
		file.close();
		printf("¡Animación cargada desde 'animacion.txt'! Frames totales: %d\n", FrameIndex);
		resetElements();
	}
	else {
...
void DoMovement()
{
	...
	if (keys[GLFW_KEY_U]) rollDog += 1.0f; 
	if (keys[GLFW_KEY_J]) rollDog -= 1.0f; 

	// Partes
	if (keys[GLFW_KEY_1]) head += 1.0f;
	if (keys[GLFW_KEY_2]) head -= 1.0f;

	if (keys[GLFW_KEY_3]) FLeftLeg += 1.0f;
	if (keys[GLFW_KEY_4]) FLeftLeg -= 1.0f;

	if (keys[GLFW_KEY_5]) FRightLeg += 1.0f;
	if (keys[GLFW_KEY_6]) FRightLeg -= 1.0f;
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
	...
	if (keys[GLFW_KEY_K])
	{
		...
		{
			printf("¡Límite máximo de 20 frames alcanzado!\n");
		}
	}
